...
function CustomTooltip({ active, payload, label }: any) {
  ...
  const periodLabel =
    d?.period === "ski" ? "Ski trip" :
    d?.period === "illness" ? "Illness" :
    d?.period === "recovery" ? "Recovery" : "At home";
  return (
    ...
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
        ...
        <YAxis
          ...
          tick={{ fontSize: 10, fill: "var(--color-faint)" }}
          stroke="var(--color-border)"
          label={{ value: "Glucose (mmol/L)", angle: -90, position: "insideLeft", style: { fontSize: 10, fill: "var(--color-muted)" } }}
        />
        <YAxis
          yAxisId="oura"
          orientation="right"
          ...
          tick={{ fontSize: 10, fill: "var(--color-faint)" }}
          stroke="var(--color-border)"
          label={{ value: "Oura Score", angle: 90, position: "insideRight", style: { fontSize: 10, fill: "var(--color-muted)" } }}
        />
        ...
        <Legend
          ...
          formatter={(value: string) => <span style={{ color: "var(--color-muted)" }}>{value}</span>}
        />
